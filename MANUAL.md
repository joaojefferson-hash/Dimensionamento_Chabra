...
**Importar do SGG.** Em vez de digitar, exporte do SGG a planilha de vencimentos (.xlsx ou .csv) e use o card *Importar do SGG* no topo da tela: escolha o arquivo, confira as colunas reconhecidas (unidade, data de vencimento, cliente, condição, porte), o ano e como contar (cada cliente uma vez por mês, ou cada linha/documento). A prévia mostra, por unidade e mês, quantos clientes vencem — e o número que está hoje ("era N"). Nomes de unidade diferentes do cadastro (acento, maiúsculas) são casados automaticamente; o que não casar aparece em vermelho para você escolher a unidade. Só ao clicar em **Importar** os números de Mensal e Exclusiva TST daquele ano, nas unidades presentes no arquivo, são substituídos (Clientes ativos não muda). Fica registrado no Histórico.
...
Como o pendente é montado, do primeiro mês em diante: pendentes = sobra do mês anterior + o que vence. Nos **meses passados**, o número lançado é o que venceu e **ainda está em aberto** — a equipe já trabalhou de verdade e o que sobrou é esse número; por isso nada é descontado e tudo passa adiante. Do **mês atual em diante**, a equipe atende o que consegue (o que limita: nos técnicos, a menor entre inspeções e relatórios) e o resto passa para o mês seguinte. Mudar o número de um mês recalcula os seguintes.
