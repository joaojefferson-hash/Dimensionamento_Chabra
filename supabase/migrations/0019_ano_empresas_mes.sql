-- ============================================================================
-- Chabra Dimensiona — 0019_ano_empresas_mes
-- Os valores por mês de "empresas com documentos vencidos" passam a ter ANO:
-- chave (unidade, ano, mês). O padrão da unidade continua valendo para qualquer
-- ano/mês sem valor próprio. Linhas existentes → 2026.
-- ============================================================================

alter table public.unidade_empresas_mes add column ano smallint not null default 2026 check (ano between 2000 and 2100);
update public.unidade_empresas_mes set ano = 2026;
alter table public.unidade_empresas_mes drop constraint unidade_empresas_mes_pkey;
alter table public.unidade_empresas_mes add primary key (unidade_id, ano, mes);
alter table public.unidade_empresas_mes alter column ano drop default;
comment on column public.unidade_empresas_mes.ano is 'Ano do valor próprio (o padrão da unidade vale para todos os anos).';

-- ---------------------------------------------------------------------------
-- importar_backup v14: empresasPorMes com ano
-- ---------------------------------------------------------------------------
...
begin
  ...
  insert into public.unidade_empresas_mes (unidade_id, ano, mes, empresas_vencidas)
  select u.id,
