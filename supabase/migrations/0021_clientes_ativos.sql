-- ============================================================================
-- Chabra Dimensiona — 0021_clientes_ativos
-- "Clientes ativos": total de clientes da unidade no mês. Só informativo
-- (histórico de carteira); NÃO entra em nenhuma conta do dimensionamento.
-- ============================================================================

alter table public.unidades add column clientes_ativos integer not null default 0 check (clientes_ativos >= 0);
comment on column public.unidades.clientes_ativos is 'Padrão: total de clientes ativos da unidade (informativo, fora das contas).';

alter table public.unidade_empresas_mes add column clientes_ativos integer not null default 0 check (clientes_ativos >= 0);
comment on column public.unidade_empresas_mes.clientes_ativos is 'Valor próprio do mês: total de clientes ativos (informativo, fora das contas).';

-- ---------------------------------------------------------------------------
-- importar_backup v16: clientesAtivos (unidade e meses)
-- ---------------------------------------------------------------------------
...
begin
  ...
  insert into public.unidades (nome, empresas_vencidas, empresas_exclusiva_tst, clientes_ativos, created_at)
  select btrim(e.elem->>'nome'),
         ...
         greatest(0, round(coalesce(nullif(e.elem->>'clientesAtivos', '')::numeric, 0)))::int,
  ...
  insert into public.unidade_empresas_mes (unidade_id, ano, mes, empresas_vencidas, empresas_exclusiva_tst, clientes_ativos)
  select u.id,
