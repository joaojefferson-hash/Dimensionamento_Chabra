-- ============================================================================
-- Chabra Dimensiona — 0020_exclusiva_tst
-- Cada unidade tem dois tipos de cliente por mês: "Mensal" (empresas_vencidas)
-- e "Exclusiva TST" (empresas_exclusiva_tst). Os dois exigem o mesmo
-- atendimento completo; a separação é para enxergar cada condição.
-- ============================================================================

alter table public.unidades add column empresas_exclusiva_tst integer not null default 0 check (empresas_exclusiva_tst >= 0);
comment on column public.unidades.empresas_vencidas is 'Padrão do ano: clientes na condição Mensal com documentos vencidos (atendimento completo).';
comment on column public.unidades.empresas_exclusiva_tst is 'Padrão do ano: clientes na condição Exclusiva TST (mesmo atendimento completo).';

alter table public.unidade_empresas_mes add column empresas_exclusiva_tst integer not null default 0 check (empresas_exclusiva_tst >= 0);
comment on column public.unidade_empresas_mes.empresas_vencidas is 'Valor próprio do mês: clientes Mensal.';
comment on column public.unidade_empresas_mes.empresas_exclusiva_tst is 'Valor próprio do mês: clientes Exclusiva TST.';

-- ---------------------------------------------------------------------------
-- importar_backup v15: empresasExclusivaTst (unidade e meses)
-- ---------------------------------------------------------------------------
...
begin
  ...
  insert into public.unidades (nome, empresas_vencidas, empresas_exclusiva_tst, created_at)
  select btrim(e.elem->>'nome'),
         greatest(0, round(coalesce(
           ...
           0)))::int,
         greatest(0, round(coalesce(nullif(e.elem->>'empresasExclusivaTst', '')::numeric, 0)))::int,
         now() + (e.n * interval '1 millisecond')
  from jsonb_array_elements(coalesce(p->'unidades', '[]'::jsonb)) with ordinality as e(elem, n)
  where btrim(coalesce(e.elem->>'nome', '')) <> '';

  -- valores por mês: com ano (backups antigos sem ano → ano corrente)
  insert into public.unidade_empresas_mes (unidade_id, ano, mes, empresas_vencidas, empresas_exclusiva_tst)
  select u.id,
