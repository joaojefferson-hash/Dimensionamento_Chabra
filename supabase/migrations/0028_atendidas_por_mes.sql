-- 0028 — Atendidas no mês: quantas empresas foram efetivamente concluídas em cada
-- unidade/mês. Diferente de clientes_ativos (informativo), este número entra no cálculo:
-- é o que sai da fila em cada mês na tela Evolução.
--   * coluna atendidas em unidade_mes
--   * importar_backup v19: lê "atendidas" nas linhas de clientesAtivosPorMes

alter table public.unidade_mes add column if not exists atendidas integer not null default 0 check (atendidas >= 0);

comment on column public.unidade_mes.atendidas is 'Empresas concluídas no mês (entra no cálculo da fila em Evolução).';
...
begin
  ...
  insert into public.unidade_mes (unidade_id, ano, mes, clientes_ativos, atendidas)
  select u.id,
         coalesce(nullif(m->>'ano', '')::int, extract(year from now())::int),
         (m->>'mes')::int,
  ...
  on conflict (unidade_id, ano, mes) do update set clientes_ativos = excluded.clientes_ativos, atendidas = excluded.atendidas;
