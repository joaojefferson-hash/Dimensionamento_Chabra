-- ============================================================================
-- Chabra Dimensiona — 0009_frequencia_atendimento
-- A cada quantos meses cada empresa da carteira recebe uma inspeção, um
-- relatório e uma finalização administrativa (padrão 1 = todo mês).
-- ============================================================================
alter table public.parametros
  add column meses_por_inspecao    numeric(5,2) not null default 1 check (meses_por_inspecao    > 0),
  add column meses_por_relatorio   numeric(5,2) not null default 1 check (meses_por_relatorio   > 0),
  add column meses_por_finalizacao numeric(5,2) not null default 1 check (meses_por_finalizacao > 0);
comment on column public.parametros.meses_por_inspecao    is 'Cada empresa recebe uma inspeção a cada N meses';
comment on column public.parametros.meses_por_relatorio   is 'Cada empresa recebe um relatório a cada N meses';
comment on column public.parametros.meses_por_finalizacao is 'Cada empresa é finalizada pelo administrativo a cada N meses';

-- importar_backup: parâmetros novos (opcionais no backup)
