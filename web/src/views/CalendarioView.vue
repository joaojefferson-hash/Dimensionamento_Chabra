...
async function folga(v) { const x = Number(v); if (!(x >= 0 && x <= 90)) { ui.toast('A folga deve ficar entre 0% e 90%.', 'error'); return; } try { await cad.atualizarParametros({ ocupacaoAlvo: 100 - x }); piscar(); } catch (e) { ui.erro(e); } }
async function prazo(v) { const x = Math.round(Number(v)); if (!(x >= 1 && x <= 365)) { ui.toast('O prazo deve ficar entre 1 e 365 dias.', 'error'); return; } try { await cad.atualizarParametros({ prazoDias: x }); piscar(); } catch (e) { ui.erro(e); } }
const rampupVal = i => (p.value.rampup[i] != null ? p.value.rampup[i] : 100);
async function rampup(i, v) {
  const x = Math.round(Number(v)); if (!(x >= 0 && x <= 100)) { ui.toast('O ramp-up deve ficar entre 0% e 100%.', 'error'); return; }
...
async function peso(codigo, v) { const x = Number(v); if (!(x >= 0.1 && x <= 20)) { ui.toast('O peso deve ficar entre 0,1 e 20.', 'error'); return; } try { await cad.atualizarPorte(codigo, { peso: x }); piscar(); } catch (e) { ui.erro(e); } }
async function restaurar() {
  const ok = await ui.confirmar({ titulo: 'Restaurar padrão 2026', mensagem: 'Substituir os 12 valores pelos dias de semana de 2026 menos os feriados nacionais?', textoConfirmar: 'Restaurar' });
  if (!ok) return;
...
<template>
  <header class="page-header"><h1>Calendário e parâmetros</h1><p>Dias de trabalho de cada mês (o ritmo diário vira produção mensal), folga para imprevistos, prazo para atender, ramp-up de quem é contratado e o peso de cada porte de cliente. Tudo é compartilhado e salva ao alterar.</p></header>
  ...
  <fieldset :disabled="!auth.podeEditar" class="contents">
    ...
    <section class="card">
      ...
      <div class="grid gap-5 md:grid-cols-3">
        <label class="text-[13px]"><span class="mb-1 block font-medium">Folga para imprevistos (%)</span><input class="input w-28" type="number" min="0" max="90" step="1" :value="Math.round(100 - p.ocupacaoAlvo)" @change="folga($event.target.value)"><small class="muted block">Com 15%, cada pessoa entrega até 85% do que declarou.</small></label>
        <label class="text-[13px]"><span class="mb-1 block font-medium">Prazo para atender (dias)</span><input class="input w-28" type="number" min="1" max="365" step="1" :value="p.prazoDias" @change="prazo($event.target.value)"><small class="muted block">O Dimensionamento diz quantas pessoas contratar para zerar o pendente nesse prazo.</small></label>
        <div class="text-[13px]"><span class="mb-1 block font-medium">Ramp-up de quem é contratado</span>
          ...
          <small class="muted block">Quanto uma pessoa nova produz nos primeiros meses de casa (depois, 100%).</small>
        </div>
      </div>
    </section>
    ...
    <section class="card">
      ...
      <p class="muted mb-3 text-[13px]">O peso multiplica o esforço de cada cliente: um cliente de peso 2 exige o trabalho de dois pequenos. Os números por porte são lançados em Empresas por Unidade.</p>
