...
<template>
  ...
  <template v-else>
    <div class="mb-5 grid gap-3 md:grid-cols-4">
      ...
      <div v-for="a in hoje.areas" :key="a.funcao" class="stat" :class="'row-' + (a.status === 'atencao' || a.status === 'deficit' ? a.status : 'ok')"><div class="label">{{ a.rotulo }}</div><div class="value">{{ num(a.cabecas) }}<small> atual</small> <span class="seta">→</span> <span :class="a.faltam > 0 ? 'txt-deficit' : 'txt-ok'">{{ a.ideal }}</span><small> ideal</small></div><div v-if="Math.abs(a.cabecas - a.pessoas) > 0.05" class="stat-detalhe muted" title="Alocação em mais de uma unidade, admissões ou desligamentos no meio do mês">equivalem a {{ numFte(a.pessoas) }} em tempo integral</div><div class="stat-detalhe">Eliminar pendências em {{ p.prazoDias }} dias: <strong :class="a.contratarPrazo > 0 ? 'txt-deficit' : 'txt-ok'">{{ a.contratarPrazo > 0 ? '+' + a.contratarPrazo : 'equipe suficiente' }}</strong></div></div>
