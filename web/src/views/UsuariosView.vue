...
const DESCRICAO = { admin: 'acesso completo; gerencia usuários e backup', supervisor: 'consulta e altera os cadastros', leitura: 'somente consulta, sem alterações' };
...
async function salvar() {
  ...
  try {
    if (editando.value) { await api.editar(editando.value, form.nome.trim(), form.sobrenome.trim()); ui.toast('Usuário atualizado.'); }
      ...
      if (form.senha.length < 8) { ui.toast('A senha deve conter no mínimo 8 caracteres.', 'error'); return; }
      ...
      ui.toast(`Usuário ${form.nome.trim()} criado. Informe a senha inicial ao usuário.`);
    }
    cancelar(); await carregar();
  } catch (e) { ui.erro(e); } finally { ocupado.value = false; }
}
async function papel(u, novo) {
  if (u.id === auth.usuario.id && novo !== 'admin') { ui.toast('Não é permitido remover o próprio perfil de administrador.', 'error'); await carregar(); return; }
  try { await api.definirPapel(u.id, novo); ui.toast(`${u.nomeCompleto || u.email}: ${PAPEIS[novo]} (válido a partir do próximo acesso).`); await carregar(); } catch (e) { ui.erro(e); await carregar(); }
}
async function senha(u) {
  const nova = gerarSenha();
  const ok = await ui.confirmar({ titulo: 'Redefinir senha', mensagem: `Definir a senha de ${u.nomeCompleto || u.email} como "${nova}"? Anote-a e informe ao usuário; ela poderá ser alterada posteriormente.`, textoConfirmar: 'Redefinir' });
  if (!ok) return;
  try { await api.redefinirSenha(u.id, nova); ui.toast(`Senha redefinida: ${nova}`); } catch (e) { ui.erro(e); }
}
async function remover(u) {
  if (u.id === auth.usuario.id) { ui.toast('Não é permitido remover a própria conta.', 'error'); return; }
...
async function importar(ev) {
  ...
  if (!json || typeof json !== 'object' || !Array.isArray(json.unidades)) { ui.toast('O arquivo não corresponde a um backup do Chabra Dimensiona.', 'error'); return; }
  const ok = await ui.confirmar({ titulo: 'Importar backup', mensagem: `Substituir TODOS os dados atuais pelo conteúdo do arquivo "${f.name}" (${json.unidades.length} unidades, ${(json.colaboradores || []).length} colaboradores)? A alteração se aplica a todos os usuários.`, textoConfirmar: 'Substituir tudo', perigo: true });
  if (!ok) return;
...
<template>
  <header class="page-header"><h1>Usuários</h1><p>Usuários com acesso ao sistema e respectivos perfis. Alterações de perfil são válidas a partir do próximo acesso.</p></header>
  ...
  <section class="card">
    ...
    <form class="grid gap-4 md:grid-cols-4" @submit.prevent="salvar">
      ...
      <template v-if="!editando">
        ...
        <div class="text-[13px] md:col-span-4"><span class="mb-1 block font-medium">Perfil</span>
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="u in lista" :key="u.id">
            <td class="font-medium">{{ u.nomeCompleto || '(sem nome)' }}<span v-if="u.id === auth.usuario.id" class="muted"> (usuário atual)</span></td>
  ...
  <section class="card">
    <div class="card-head"><h2>Backup dos dados</h2><span class="muted">os dados são armazenados em nuvem e compartilhados com toda a equipe</span></div>
    <p class="muted mb-3 text-[13px]">Exporte um arquivo JSON com todos os cadastros para manter uma cópia de segurança. A importação <strong>substitui todos os dados atuais</strong> — utilize-a apenas para restaurar um backup.</p>
