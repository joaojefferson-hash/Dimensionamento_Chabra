...
export function montarMes(demanda = {}, clientesAtivos = 0, atendidas = 0, atendidasPorte = null) {
  ...
  const porte = {};
  Object.entries(atendidasPorte || {}).forEach(([k, v]) => { const q = Math.max(0, Math.round(num(v))); if (q > 0) porte[k] = q; });
  const totalAtendidas = Object.keys(porte).length ? Object.values(porte).reduce((s, q) => s + q, 0) : Math.max(0, Math.round(num(atendidas)));
  ...
  return { demanda: d, ...contagens, clientesAtivos: Math.max(0, Math.round(num(clientesAtivos))), atendidas: totalAtendidas, atendidasPorte: porte };
}

/** Linhas de demanda_mensal + unidade_mes → mesesPorAno de cada unidade. */
function mesesPorUnidade(demanda, ativos) {
  const por = {};
  const slot = (uid, ano, mes) => { const u = (por[uid] = por[uid] || {}); const a = (u[ano] = u[ano] || {}); return (a[mes] = a[mes] || { demanda: {}, clientesAtivos: 0, atendidas: 0, atendidasPorte: {} }); };
  ...
  ativos.forEach(a => { const s = slot(a.unidade_id, a.ano, a.mes); s.clientesAtivos = num(a.clientes_ativos); s.atendidas = num(a.atendidas); s.atendidasPorte = a.atendidas_porte || {}; });
  Object.values(por).forEach(anos => Object.values(anos).forEach(meses => Object.keys(meses).forEach(m => { meses[m] = montarMes(meses[m].demanda, meses[m].clientesAtivos, meses[m].atendidas, meses[m].atendidasPorte); })));
  return por;
}
...
export async function carregarTudo() {
  ...
  const [funcoes, unidades, colaboradores, alocacoes, demanda, ativos, portes, parametros, clientesPorte] = await Promise.all([
    ...
    q(supabase.from('unidade_mes').select('unidade_id, ano, mes, clientes_ativos, atendidas, atendidas_porte'), 'os clientes ativos e atendimentos'),
...
export const demanda = {
  ...
  async definirUnidadeMes(unidadeId, ano, mes, { clientesAtivos = 0, atendidasPorte = null } = {}) {
    const ativos = Math.max(0, Math.round(num(clientesAtivos)));
    const porte = {};
    Object.entries(atendidasPorte || {}).forEach(([k, v]) => { const q = Math.max(0, Math.round(num(v))); if (q > 0) porte[k] = q; });
    const total = Object.values(porte).reduce((s, q) => s + q, 0);
    const r = ativos > 0 || total > 0
      ? await supabase.from('unidade_mes').upsert({ unidade_id: unidadeId, ano, mes, clientes_ativos: ativos, atendidas: total, atendidas_porte: porte }, { onConflict: 'unidade_id,ano,mes' })
    ...
    return { clientesAtivos: ativos, atendidas: total, atendidasPorte: porte };
  },
